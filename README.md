...
2. Создайте собственное OAuth-приложение Яндекса по инструкции ниже и соберите APK со своим Client ID, если используете исходный код.
3. Запустите приложение на часах и продолжите вход на телефоне либо непосредственно на часах.
4. Добавьте плитки «Умный дом» и «Свет» через системный редактор Wear OS.
...
- Android Studio с поддержкой AGP 9.3.x;
- JDK 17;
- Android SDK Platform 37 и актуальные Build Tools;
- Wear OS 3+ часы или эмулятор с доступом в интернет;
- для phone OAuth — сопряжённый Android-телефон с Google Play Services;
- OAuth-приложение Яндекса с правами `iot:view` и `iot:control`.

## Регистрация OAuth-приложения Яндекса
